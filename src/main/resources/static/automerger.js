...
Gerrit.install(function(self) {
    var currentUserEmail = Gerrit.getCurrentUser().email;

    function abandonChange(changeId) {
        var abandonInput = {
          "message": `Automerger abandoning and recreating due to \
                      ${currentUserEmail} clicking skip in UI`,
          "notify": "NONE"
        };
    ...
    function skipDownstream() {
        ...
        var reviewInput = {
            "message": `Downstream changes skipped by ${currentUserEmail}`
        };

        var reviewUrl = `/changes/${currentChange.id}/revisions` +
                        `/${currentChange.current_revision}/review`
        Gerrit.post(reviewUrl, reviewInput, function(r) {
            Gerrit.refresh();
        });
    }
