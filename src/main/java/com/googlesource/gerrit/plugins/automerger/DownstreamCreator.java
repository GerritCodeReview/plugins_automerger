...
public class DownstreamCreator
  ...
  public void onChangeRestored(ChangeRestoredListener.Event event) {
    try (ManualRequestContext ctx = oneOffRequestContext.openAs(config.getContextUserId())) {
      ChangeInfo change = event.getChange();
      automergeChanges(createMdsMergeInput(change, event.getRevision()));
   ...
   */
  public void automergeChanges(MultipleDownstreamMergeInput mdsMergeInput)
      throws RestApiException, ConfigInvalidException, InvalidQueryParameterException, OrmException,
          IOException {
    if (mdsMergeInput == null) {
      return;
    }
    try (ManualRequestContext ctx = oneOffRequestContext.openAs(config.getContextUserId())) {
      ReviewInput reviewInput = new ReviewInput();
      ...
      ChangeInfo currentChange = gApi.changes().id(mdsMergeInput.changeNumber).get();
      propagateVotes(currentChange);
    }
  }
  ...
  private MultipleDownstreamMergeInput createMdsMergeInput(
      ChangeInfo change, RevisionInfo revisionInfo)
  ...
  private void propagateVotes(ChangeInfo change)
      throws RestApiException, OrmException, ConfigInvalidException, IOException {
    log.info("propgateVotes called for " + change.branch);
    String revision = change.currentRevision;
    Set<String> downstreamBranches;
    downstreamBranches = config.getDownstreamBranches(change.branch, change.project);
    log.info("Downstream branches: " + downstreamBranches);
    ...
    Map<String, LabelInfo> labels =
        gApi.changes().id(change._number).get(EnumSet.of(ListChangesOption.DETAILED_LABELS)).labels;
