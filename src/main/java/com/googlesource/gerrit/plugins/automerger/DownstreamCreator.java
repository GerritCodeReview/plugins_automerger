...
public class DownstreamCreator
        ...
        try {
          ...
          for (Integer changeNumber : existingDownstream) {
            ChangeInfo downstreamChange =
                apiManager.forHostname(toCrossHost).changes().id(changeNumber).get(EnumSet.of(ListChangesOption.CURRENT_REVISION));
            for (Map.Entry<String, LabelInfo> labelEntry : labels.entrySet()) {
              if (labelEntry.getValue().all.size() > 0) {
                ...
                if (maxVote.isPresent()) {
                  updateVote(downstreamChange, labelEntry.getKey(), (short) maxVote.getAsInt(), toCrossHost);
                }
              }
            }
          }
  ...
  public void createMergesAndHandleConflicts(MultipleDownstreamMergeInput mdsMergeInput)
    ...
    try (ManualRequestContext ctx = oneOffRequestContext.openAs(config.getContextUserId())) {
      ...
      ChangeInfo originalChange =
          getOriginalChange(mdsMergeInput.changeNumber, mdsMergeInput.currentRevision, mdsMergeInput.sourceBranch, null);
      // if this fails, i.e. -2 is restricted, catch it and still post message without a vote.
      try {
        apiManager.forHostname(mdsMergeInput.fromCrossHostMap.get(originalChange.branch)).changes().id(originalChange._number).revision(CURRENT).review(reviewInput);
        ...
        if (mdsMergeInput.obsoleteRevision != null) {
          existingDownstream =
              getExistingMergesOnBranch(
                  mdsMergeInput.obsoleteRevision, mdsMergeInput.topic, downstreamBranch, toCrossHost);
    ...
    try (ManualRequestContext ctx = oneOffRequestContext.openAs(config.getContextUserId())) {
      ...
      downstreamChangeInput.baseChange =
          getBaseChangeId(
              ...
              sdsMergeInput.downstreamBranch, sdsMergeInput.fromCrossHost, sdsMergeInput.toCrossHost);
      ...
      ChangeApi downstreamChange = apiManager.forHostname(sdsMergeInput.toCrossHost).changes().create(downstreamChangeInput);
   ...
   * @throws RestApiException
   */
  private String getBaseChangeId(List<String> parents, String branch, String fromCrossHost, String toCrossHost)
    ...
    List<ChangeInfo> changesInTopic = getChangesInTopicAndBranch(firstParentTopic, branch, toCrossHost);
  ...
  private void automergeChanges(ChangeInfo change, RevisionInfo revisionInfo)
    ...
    for (String downstreamBranch : downstreamBranches) {
      ...
      if (fromCrossHost != null) {
        fromCrossHostMap.put(change.branch, fromCrossHost);
      }
      if (toCrossHost != null) {
        toCrossHostMap.put(downstreamBranch, toCrossHost);
      }
    }
    ...
    mdsMergeInput.fromCrossHostMap = fromCrossHostMap;
    mdsMergeInput.toCrossHostMap = toCrossHostMap;

    createMergesAndHandleConflicts(mdsMergeInput);
  }

  private void updateVote(ChangeInfo change, String label, short vote, String hostname) throws RestApiException {
    ...
    try {
      apiManager.forHostname(hostname).changes().id(change.id).revision(CURRENT).review(reviewInput);
    } catch (AuthException e) {
      log.error("Automerger could not set label, but still continuing.", e);
    }
  }

  private void tagChange(ChangeInfo change, String message, String hostname) throws RestApiException {
    ReviewInput reviewInput = new ReviewInput();
  ...
  private void updateDownstreamMerge(
    ...
    mergePatchSetInput.baseChange =
        getBaseChangeId(
            getChangeParents(upstreamChangeNumber, newParentRevision), downstreamBranch, fromCrossHost, toCrossHost);

    ChangeApi originalChange = apiManager.forHostname(toCrossHost).changes().id(existingDownstreamNum);
  ...
  private ChangeInfo getOriginalChange(int changeNumber, String currentRevision, String downstreamBranch, String hostname)
    ...
    if (parents.size() >= 2) {
      ...
      for (ChangeInfo change : changesInTopic) {
        ...
        if (change.currentRevision.equals(secondParentRevision)) {
          return getOriginalChange(change._number, secondParentRevision, change.branch, newHostname);
        }
      }
    }
  ...
  private String getTopic(String revision, String hostname) throws InvalidQueryParameterException, RestApiException {
    QueryBuilder queryBuilder = new QueryBuilder();
    queryBuilder.addParameter("commit", revision);
    List<ChangeInfo> changes =
        apiManager.forHostname(hostname).changes()
  ...
  private List<ChangeInfo> getChangesInTopic(String topic, String hostname)
    ...
    return apiManager.forHostname(hostname).changes()
        .query(queryBuilder.get())
        .withOptions(ListChangesOption.ALL_REVISIONS, ListChangesOption.CURRENT_COMMIT)
        .get();
  }

  private List<ChangeInfo> getChangesInTopicAndBranch(String topic, String downstreamBranch, String hostname)
      throws InvalidQueryParameterException, RestApiException {
    QueryBuilder queryBuilder = constructTopicQuery(topic);
  ...
  private boolean isAlreadyMerged(SingleDownstreamMergeInput sdsMergeInput, String currentTopic)
    ...
    List<ChangeInfo> changes =
        getChangesInTopicAndBranch(currentTopic, sdsMergeInput.downstreamBranch, sdsMergeInput.toCrossHost);
    for (ChangeInfo change : changes) {
