...
public class DownstreamCreator
  ...
  public void createMergesAndHandleConflicts(MultipleDownstreamMergeInput mdsMergeInput)
    ...
    try (ManualRequestContext ctx = oneOffRequestContext.openAs(config.getContextUserId())) {
      ...
      try {
        apiManager
            .forHostname(mdsMergeInput.usBranchToHostMap.get(originalChange.branch))
            .changes()
      ...
      for (String downstreamBranch : mdsMergeInput.dsBranchMap.keySet()) {
          ...
          if (!existingDownstream.isEmpty()) {
            ...
            for (Integer dsChangeNumber : existingDownstream) {
              try {
                ...
                log.info("mds merge input crosshmap: {}", mdsMergeInput.dsBranchToHostMap);
                log.info(
                    "mds merge input crosshmap: {}",
                    mdsMergeInput.dsBranchToHostMap.get(downstreamBranch));
                updateDownstreamMerge(
                    ...
                    downstreamBranch,
                    mdsMergeInput.dsBranchToHostMap.get(downstreamBranch),
                    mdsMergeInput.usBranchToHostMap.get(mdsMergeInput.sourceBranch));
        ...
        if (createDownstreams) {
          ...
          sdsMergeInput.fromCrossHost =
              mdsMergeInput.usBranchToHostMap.get(mdsMergeInput.sourceBranch);
          sdsMergeInput.toCrossHost = mdsMergeInput.dsBranchToHostMap.get(downstreamBranch);
          try {
  ...
  private void automergeChanges(ChangeInfo change, RevisionInfo revisionInfo)
    ...
    mdsMergeInput.usBranchToHostMap =
        config.getUpstreamBranchesToHostMap(change.branch, change.project);
    mdsMergeInput.dsBranchToHostMap =
        config.getDownstreamBranchesToHostMap(change.branch, change.project);
