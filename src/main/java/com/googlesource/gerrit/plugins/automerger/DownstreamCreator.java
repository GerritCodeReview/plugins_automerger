...
public class DownstreamCreator
  ...
  public void onTopicEdited(TopicEditedListener.Event event) {
    ...
    // If change is empty, or change is automerger-created change, prevent someone breaking topic.
    if (isNullOrEmpty(change.topic) || change.subject.startsWith(SUBJECT_PREFIX)) {
      try {
        gApi.changes().id(change._number).topic(oldTopic);
        // TODO(stephenli): leave a comment on the change so that user knows what's going on
