...
public class DownstreamCreator
  ...
  private boolean shouldReceivePropagatedVote(ChangeInfo downstreamChange)
    ...
    if (config.contextUserIdIsSet()) {
      Integer accountId =
          downstreamChange.revisions.get(downstreamChange.currentRevision).uploader._accountId;

      shouldPropagate = accountId == config.getContextUserId().get();
    }
    // Check that the change does not have any downstream conflicts
    shouldPropagate =
        shouldPropagate && mergeValidator.getMissingDownstreamMerges(downstreamChange).isEmpty();
