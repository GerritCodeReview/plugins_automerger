...
public class ConfigLoaderIT {
  ...
  private void mockFile(String resourceName, String projectName,
                           String branchName, String filename)
      throws Exception {
    try (InputStream in =
             getClass().getResourceAsStream(resourceName)) {
      String resourceString = CharStreams.toString(
          new InputStreamReader(in, Charsets.UTF_8));
      Mockito.when(gApiMock.projects().name(projectName)
                       .branch(branchName).file(filename).asString())
  ...
  public void isSkipMergeTest_blankMergeWithMergeAll() throws Exception {
    loadConfig();
    assertThat(configLoader.isSkipMerge(
        "master", "ds_two", "test test \n \n DO NOT MERGE")).isFalse();
  }
  ...
  public void isSkipMergeTest_alwaysBlankMerge() throws Exception {
    loadConfig();
    assertThat(configLoader.isSkipMerge(
        "master", "ds_one", "test test \n \n DO NOT MERGE ANYWHERE")).isTrue();
  }
  ...
  public void downstreamBranchesTest_IOException() throws Exception {
    Mockito.when(gApiMock.projects().name("platform/manifest")
                     .branch("master").file("default.xml").asString())
